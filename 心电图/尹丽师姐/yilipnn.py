...
path_train = r'C:\Users\liuju\Desktop\PNN_python-master\数据\分类\data.xlsx'
path_trainlab = r'C:\Users\liuju\Desktop\PNN_python-master\数据\分类\label_data.xlsx'
train_data = import_excel_matrix(path_train)
train_label = import_excel_matrix(path_trainlab)


path_test = r'C:\Users\liuju\Desktop\PNN_python-master\数据\分类\testdata.xlsx'
path_testlab = r'C:\Users\liuju\Desktop\PNN_python-master\数据\分类\testlabel_data.xlsx'
test_data = import_excel_matrix(path_test)
test_label = import_excel_matrix(path_testlab)
...
# 1、导入数据
print ("--------- 1.load data ------------")
trainX, labelX = train_data,train_label


# 2、样本数据归一化
Nor_trainX = Normalization(trainX)
Nor_testX = Normalization(test_data) 
# 3、计算Gauss矩阵
...
def start_train(sigm=0.1):
    ...
    for c in range(len(Nor_testX)):
        if predict_results[c] == test_label[c]:
            num =num +1
