...
if __name__ == '__main__':
    ...
    SA = SA_Tabu(L=L,t=0.0001,T=300,each_T_internations=200,
                        ...
                        threshold_t=10,re_T=80,len_list=10)
    out_value,out_blade_list,out_blade_weight,out_all_list=SA.run
    ...
    d_store,count = {},0
    d_store['min_value']=min(out_value)
    d_store['value']=out_value 
    for value in out_value:
        d_store['叶片_%s'%count]     = out_blade_list[count]
        d_store['重量_%s'%count] = out_blade_weight[count]
        count+=1
