...
MM=[];
GGoal=[];
min_goal=[];%小于某个阈值，将结果存放在这个矩阵
min_store=[];%将min_goal的值对应的序列存放在这个矩阵
...
while T>t
    ...
    for i=1:K
        ...
        if goal_new<0.007 && ~any(min_goal==goal_new)
            min_goal=[min_goal,goal_new];
            min_store=[min_store,M];
        end
        detaT=goal_new-goal;
        if detaT<0
            L=M;
